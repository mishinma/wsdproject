...
def run():
    ...
    # Game categories
    rpg_cat = get_or_create_game_cat('RPG')
    action_cat = get_or_create_game_cat('Action')
    strategy_cat = get_or_create_game_cat('Strategy')
    fps_cat = get_or_create_game_cat('FPS')
    # Users
    jon = get_or_create_user('jon', 'jon@snow.west', 'jon', True)
    sansa = get_or_create_user('sansa', 'sansa@stark.west', 'sansa', True)
    arya = get_or_create_user('arya', 'arya@stark.west', 'arya', True)
    bran = get_or_create_user('bran', 'bran@stark.west', 'bran')
    robb = get_or_create_user('robb', 'robb@stark.west', 'robb')
    ned = get_or_create_user('ned', 'ned@stark.west', 'ned')
    # Games
    game1 = get_or_create_game(jon, rpg_cat, default_url, 10.0,
                               'The Battle of the Bastards 1',
                               default_desc, 4.5)
    game2 = get_or_create_game(jon, rpg_cat, default_url, 20.0,
                               'The Battle of the Bastards 2',
                               default_desc, 4.5)
    game3 = get_or_create_game(jon, action_cat, default_url, 30.0,
                               'The Battle of the Bastards 3',
                               default_desc, 4.5)
    game4 = get_or_create_game(sansa, action_cat, default_url, 40.0,
                               'The Battle of the Bastards 4',
                               default_desc, 4.5)
    game5 = get_or_create_game(sansa, action_cat, default_url, 50.0,
                               'The Battle of the Bastards 5',
                               default_desc, 4.5)
    game6 = get_or_create_game(sansa, strategy_cat, default_url, 60.0,
                               'The Battle of the Bastards 6',
                               default_desc, 4.5)
    game7 = get_or_create_game(arya, strategy_cat, default_url, 70.0,
                               'The Battle of the Bastards 7',
                               default_desc, 4.5, 50.0)
    game8 = get_or_create_game(arya, strategy_cat, default_url, 80.0,
                               'The Battle of the Bastards 8',
                               default_desc, 4.5, 60.0)
    game9 = get_or_create_game(arya, fps_cat, default_url, 90.0,
                               'The Battle of the Bastards 9',
                               default_desc, 4.5, 70.0)
    game10 = get_or_create_game(jon, fps_cat, default_url, 100.0,
